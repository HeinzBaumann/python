...
# Exercise 3: how many vendors have a date retention of 180 days or more?
# Extra: print the vendors with a data retention bigger than 180 days
    ...
    if len(dict):
        ...
        for x in dict['purposes']:
            if dict.get('purposes') != None and dict['purposes'][x] > value:
                return True
        for x in dict['specialPurposes']:
            if dict.get('specialPurposes') != None and dict['specialPurposes'][x] > value:
                return True
        return False
...
for x in dictAvis:
    if dictAvis[x]['internationalTransfers'] and dictAvis[x]['internationalTransfers'] == True:
        outStr = outStr + dictAvis[x]['name'] + '\t'
        cnt += 1
        print(dictAvis[x]['transferMechanisms'])
        if dictAvis[x]['transferMechanisms']:
            i = 1
            for y in dictAvis[x]['transferMechanisms']:
                outStr = outStr + y
                print(i, len(dictAvis[x]['transferMechanisms']))
                if (i < len(dictAvis[x]['transferMechanisms'])):
                    outStr += ', '
                    i += 1
        outStr += '\n'
print(outStr)
...
outStr = "List of CMPs supporting Native App (Mobile): \n"
cnt = 0
